...
{
    public class ARMA2OA // SteamCMDAgent is not used because ARMA2 OA relies on SteamCMD but needed base game for installation and update process
    {
        // Standard variables
        ...
        {
            name = "WindowsGSM.ARMA2OA", // WindowsGSM.XXXX
            author = "GTVolk",
        ...
        public ARMA2OA(Functions.ServerConfig serverData) => this.serverData = serverData;

        // - Settings properties for SteamCMD installer
        public bool loginAnonymous => false; // ARMA2 OA requires to login steam account to install the server, so loginAnonymous = false
        public string GameId => "33930"; // Game server appId, ARMA2 OA is 33935
        public string AppId => "33935"; // Game server appId, ARMA2 OA is 33935


        // - Game server Fixed variables
        public string StartPath => "arma2oaserver.exe"; // Game server start path, for ARMA2 OA, it is arma2oaserver.exe
        public string FullName = "Arma 2 Operation Arrowhead Dedicated Server"; // Game server FullName
        ...
        public async Task Stop(Process p) => await Task.Run(() => { p.Kill(); }); // I believe ARMA2 OA don't have a proper way to stop the server so just kill it
